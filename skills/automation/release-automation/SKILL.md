---
    name: release-automation
    description: Automate versioning, changelogs, tags, artifacts, and release notes safely.
    argument-hint: "[scope | file | goal]"
    metadata:
      origin: VEX
      category: automation
      triggers: ["Release scripts", "Tag automation", "Changelog generation"]
    ---

    # Release Automation

    Automate versioning, changelogs, tags, artifacts, and release notes safely.

    ## When to Activate

    - Release scripts
- Tag automation
- Changelog generation

    ## How It Works

    1. Clarify scope and success criteria before changing files.
    2. Inspect existing project conventions and reuse local tooling first.
    3. Apply smallest safe change that satisfies requirement.
    4. Validate with targeted commands, tests, or manual checks.
    5. Report result with changed paths, evidence, and remaining risk.

    ## Examples

    - `/release-automation src/api` — apply workflow to specific path.
    - `/release-automation failing checkout test` — focus on named issue.
    - `/release-automation` — infer scope from current diff or task.

    ## Critical Callouts

    - Do not bypass failing quality gates; fix root cause.
    - Prefer project-owned scripts over remote one-off commands.
    - Validate external input, secrets, and shared-state changes carefully.
    - Stop and ask before destructive or externally visible actions.

    ## Related Skills

    - `ci-cd-setup`
- `github-actions`
- `docker-compose`
- `pre-commit-hooks`

